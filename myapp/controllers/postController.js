...
// Mostra todos os posts de uma thread
  ...
  Post.find({ thread: threadId })
    ...
    .exec((err, dbposts) => {   
      if (err) {
        ...
        res.status(404).send("Nenhum post encontrado para essa thread");
      } else {
        res.status(200).send(dbposts);
      }
    });
...
postController.toggleHidePost = function (req, res) {
  Post.findById(req.params.id, function (err, post) {
    if (err) {
      console.log("Erro ao encontrar o post");
      res.status(500).send(err);
    } else {
      if (
        post.creator.toString() !== req.userId &&
        ...
        post.save(function (err) {
          if (err) {
            console.log("Erro ao ocultar/descobrir o post");
            res.status(500).send(err);
          } else {
            res.status(200).send(post);
          }
